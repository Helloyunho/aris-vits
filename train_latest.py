...
def train_and_evaluate(
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(
        ...
        with torch.autocast(device_type=device.type, enabled=hps.train.fp16_run):
            (
                y_hat,
                y_hat_mb,
                l_length,
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(
            ...
            break
        y_hat, y_hat_mb, attn, mask, *_ = generator.infer(x, x_lengths, max_len=1000)
...
if __name__ == "__main__":
    with open("model/aris_ms_istft_vits.json", "r") as f:
        data = f.read()
    config = json.loads(data)
    hps = utils.HParams(**config)
    hps.model_dir = "./logs/aris"
    run(hps)
